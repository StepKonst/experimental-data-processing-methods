...
def main():
    ...
    model_utils.plot_line_chart(
        lpw, "Время", "Значение импульсной реакции", "darkorange"
    )

    st.subheader("График фильтра низких частот (2 * m + 1 Potter)")
    model_utils.plot_line_chart(
        ref_lpw, "Время", "Амплитуда импульсной реакции", "darkorange"
    )
    ...
    model_utils.plot_chart_two_params(
        new_X_n, tf_ref_lpw, "Частота", "Амплитуда импульсной реакции", "darkorange", 2
    )

    st.subheader("График фильтра высоких частот ФВЧ")
    model_utils.plot_line_chart(
        hpf, "Время", "Значение импульсной реакции", "darkorange"
    )

    st.subheader("График частотной характеристики фильтра высоких частот")
    tf_hpf = tf_hpf[: len(tf_hpf) // 2]
    model_utils.plot_chart_two_params(
        new_X_n, tf_hpf, "Частота", "Амплитуда импульсной реакции", "darkorange", 2
    )

    st.subheader("График полосового фильтра ПФ")
    model_utils.plot_line_chart(
        bpf, "Время", "Значение импульсной реакции", "darkorange"
    )

    st.subheader("График частотной характеристики полосового фильтра")
    tf_bpf = tf_bpf[: len(tf_bpf) // 2]
    model_utils.plot_chart_two_params(
        new_X_n, tf_bpf, "Частота", "Амплитуда импульсной реакции", "darkorange", 2
    )

    st.subheader("График режекторного фильтра РФ")
    model_utils.plot_line_chart(
        bsf, "Время", "Значение импульсной реакции", "darkorange"
    )

    st.subheader("График частотной характеристики режекторного фильтра")
    tf_bsf = tf_bsf[: len(tf_bsf) // 2]
    model_utils.plot_chart_two_params(
        new_X_n, tf_bsf, "Частота", "Амплитуда импульсной реакции", "darkorange", 2
    )
